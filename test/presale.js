const {
  ...
  describe('eesee', function () {
    ...
    await ESE.transfer(presaleCrowdsale.address, ethers.utils.parseUnits('1000', 'ether'))
    ...
    it('Can\'t buy tokens less than minimum', async () => {
      ...
      await expect(presaleCrowdsale.connect(acc2).buyTokens(acc2.address, getProof(merkleTree, acc2.address), {value: ethers.utils.parseEther('0.00001')}))
    ...
    it('Can\'t buy tokens if not in whitelist', async () => {
      await expect(presaleCrowdsale.connect(signer).buyTokens(signer.address, getProof(merkleTree, acc2.address), {value: ethers.utils.parseEther('0.001')}))
    ...
    it('Can buy tokens', async () => {
      ...
      await expect(presaleCrowdsale.connect(acc2).buyTokens(acc2.address, getProof(merkleTree, acc2.address), {value: ethers.utils.parseEther('0.0001')}))
    ...
    it('Can\'t buy more than cap', async () => {
      await expect(presaleCrowdsale.connect(acc2).buyTokens(acc2.address, getProof(merkleTree, acc2.address), {value: ethers.utils.parseEther('0.00091')}))
    ...
    it('Tokens locked correctly after purchase', async () => {
      ...
      const lockedTokens = await ESE.getLockedUserTokens(acc2.address, 0)
      assert.equal(presalePrivateLockedTokens.toString(), ethers.utils.parseEther('2').toString(), 'presalePrivateLockedTokens is correct')
      assert.equal(presaleLiquidityLockedTokens.toString(), ethers.utils.parseEther('5').toString(), 'presaleLiquidityLockedTokens is correct')
      assert.equal(lockedTokensAmount.toString(), ethers.utils.parseEther('3').toString(), 'lockedTokensAmount is correct')
      assert.equal(lockedTokens.amount.toString(), ethers.utils.parseEther('3').toString(), 'lockedTokens amount is correct')
      assert.equal(lockedTokens.unlockTimestamp.toString(), (buyTokensTimestamp + 31536000) + '', 'lockedTokens unlockTimestamp is correct')
    })
